...
const T6_TRADERS = {
  ...
  "South": ["Gr├índiha", "Starry Midnight Port"],
...
// Hakoven before Arehaza (East), Gr├índiha before Starry Midnight Port (South).
...
// Real T6/T7 item names when the trade carries them (from a scanned T6ΓåÆT7
...
// "Gr├índiha" with the unaccented "Grandiha" a scanned/typed chain may carry).
function nameKey(s) {
...
export function generateAllConfigs(trades, ilyaStock, regionMapping, allStock = false) {
  ...
  // allStock=true ΓåÆ all active regions; an object (e.g. { east: true }) ΓåÆ the
 ...
 *  - Each remaining region is loaded with T4 at Iliya, then T4ΓåÆT5 ΓåÆ T5ΓåÆT6 ΓåÆ T6ΓåÆT7.
...
function buildOptimizedRoute(config, trades, regionMapping, shipCapacity = 22450, characterBaseWeight = 5400, characterUsedWeight = 150, ports = null) {
  ...
  // Player inventory threshold: (baseWeight ├ù 1.7) - usedWeight.
  ...
  // Trade inputΓåÆoutput in the ship (mirrors Simulator.trade)
  ...
  // Trade a region's T6ΓåÆT7 sets. With region-based T6 names, trades that share
  // the same "[Level 6] {Region} ΓåÆ [Level 7] {Region}" barter must be spread one
  ...
  const processRegionChain = (region, source) => {
    ...
    // Small-ship support: if the region's combined T6 output (n ├ù 5 ├ù 2000lt)
    ...
    // T4ΓåÆT5 at T5 islands (skip for Ilya stock, which is already T5)
    if (source === 't4') {
    ...
    // T5ΓåÆT6 at T6 traders (each trade bartered at its own trader)
    ...
    // T6ΓåÆT7: barter them all at their offering ports, then sell the T7s at the
    // last port - consecutive T6ΓåÆT7s don't change ship weight, so defer sells.
    for (let i = 0; i < regionTrades.length; i++) {
      const tradeData = regionTrades[i];
  ...
  // Re-barter a stocked region's T4ΓåÆT5 at its islands and return the produced
 ...
 *
 * Strategy: load T4 for ALL non-Ilya regions at once, barter every T4ΓåÆT5 in a
 * single loop, then process each region's T5ΓåÆT6 ΓåÆ T6ΓåÆT7 ΓåÆ sell chain. Items
 ...
 *
 * Feasibility: a region's T6 output (5 ├ù trades ├ù 2000lt) plus the few
...
function buildJugglingRoute(config, trades, regionMapping, shipCapacity = 22450, characterBaseWeight = 5400, characterUsedWeight = 150, ports = null) {
  ...
  // regions' T5s to the player so this region's T6 output (n ├ù 5 ├ù 2000lt) fits
  ...
  // T6ΓåÆT7 barters followed by a deferred sell. Consecutive T6ΓåÆT7 trades don't
  ...
  // Run a region's T5ΓåÆT6 ΓåÆ T6ΓåÆT7 ΓåÆ sell chain with the ship already holding the
  ...
  // T5ΓåÆT6 ΓåÆ T6ΓåÆT7 ΓåÆ sell chain.
  ...
  // after A's T5ΓåÆT6: overstack A's last T6 stack into the player (only one T6
  // stack fits the player; with a single trade the T6 stays on the ship), load
  // B's T4s into the freed space, then run A's T6ΓåÆT7 + sell interleaved with
  // B's island T4ΓåÆT5 sweep. This avoids a separate return trip to Iliya to load
  // B's T4s.
  const processOverstackHandoff = (A, A_trades, B_regions, B_trades) => {
    ...
    // still accepted). With ΓëÑ2 trades, move the last one to the player to free
    ...
    // Sweep B's islands (T4ΓåÆT5) while travelling toward A's last T7 port.
    const bSweep = [];
  ...
  // order from the current location, then barter each T4ΓåÆT5. When a post-sweep
  ...
  if (stockRegions.length === 1 && nonStockRegions.length > 0) {
    ...
    // ShipΓåöplayer swaps can only happen at Iliya, Lema, Kuit Islands and the
    // T5ΓåÆT6 / T6ΓåÆT7 trader ports - never at a remote T4ΓåÆT5 island. Kuit Islands
  ...
  // Non-combined flow: non-stock regions (T4 ΓåÆ islands ΓåÆ chains, batched), with
  ...
  if (nonStockRegions.length > 0) {
    ...
    for (let gi = 0; gi < groups.length; gi++) {
      ...
      // swept its islands - just run the T5ΓåÆT6 ΓåÆ T6ΓåÆT7 ΓåÆ sell chains.
      ...
      // Sweep the T4ΓåÆT5 islands, ending closest to the first T6 trader of the
      ...
      // last region (interleaves its T6ΓåÆT7 with the next group's load, skipping
      ...
      if (nextGroup && nextGroupTrades && lastRegionTrades.length >= 1) {
        ...
        // Process each region's T5ΓåÆT6 ΓåÆ T6ΓåÆT7 ΓåÆ sell
        const batchResult = processBatchedRegions(group, groupTrades);
        if (!batchResult.success) return batchResult;
      }
    }
...
function refineRoute(route, actions, trades, shipCapacity, characterBaseWeight, characterUsedWeight, ports) {
  ...
  // The fixed T6 sailing orders (HakovenΓåÆArehaza East, Gr├índihaΓåÆStarry South)
